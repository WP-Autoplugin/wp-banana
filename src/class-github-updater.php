...
class GitHub_Updater {
	...
	public function get_new_version() {
		...
		if ( $this->overrule_transients() || ( ! isset( $version ) || ! $version || '' === $version ) ) {
			...
			if ( false === $version && ( ! empty( $this->config['readme'] ) && 'readme.txt' !== $this->config['readme'] ) ) {
	...
	public function pre_plugin_information() {
		$plugin = isset( $_GET['plugin'] ) ? sanitize_text_field( wp_unslash( $_GET['plugin'] ) ) : ''; // phpcs:ignore WordPress.Security.NonceVerification -- read-only, non-destructive.
		...
		echo '<div class="wrap">';
		echo '<h2>' . esc_html__( 'Plugin Information', 'wp-banana' ) . '</h2>';
		echo '<p>' . esc_html__( 'See the GitHub page for details.', 'wp-banana' ) . '</p>';
		echo '<p><a href="' . esc_url( $github ) . '" target="_blank" rel="noopener noreferrer">' . esc_html__( 'GitHub Repository', 'wp-banana' ) . '</a></p>';
		echo '</div>';
	...
	public function upgrader_post_install( $true, $hook_extra, $result ) { // phpcs:ignore
		...
		$fail_message    = __( 'The plugin has been updated, but could not be reactivated. Please reactivate it manually.', 'wp-banana' );
		$success_message = __( 'Plugin reactivated successfully.', 'wp-banana' );
